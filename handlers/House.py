...
class AreaInfoHandler(BaseHandler):
    ...
    def get(self):
        ...
        if ret:
            ...
            resp = dict(errcode=RET.OK, errmsg="OK", data=json.loads(ret))
...
class HouseIMageHandler(BaseHandler):
    ...
    def post(self):
        ...
        if not img_name:
            return self.write(dict(errcode=RET.THIRDERR, errmsg="七牛接口錯誤"))
        try:
            # 保存圖片路徑到數據庫ih_house_image表,並且設置房屋的主圖片(ih_house_info中的hi_index_image_url)
            # 我們將用戶上傳的第一張圖片作為房屋的主頁
            ...
            logging.error(e)
            return self.write(dict(errcode=RET.DBERR, errmsg="上傳失敗"))
...
class HouseInfoHandler(BaseHandler):
    ...
    def post(self):
        ...
        if not all((title, price, area_id, address, room_count, acreage, unit, capacity, beds, deposit, min_days,
                    max_days)):
            return self.write(dict(errcode=RET.PARAMERR, errmsg="缺少參數"))

        try:
            price = int(price) * 100
            deposit = int(deposit) * 100
        except Exception as e:
            return self.write(dict(errcode=RET.PARAMERR, errmsg="參數錯誤"))

        # 數據
        try:
            sql = "insert into ih_house_info(hi_user_id,hi_title,hi_price,hi_area_id,hi_address,hi_room_count," \
                  "hi_acreage,hi_house_uint,hi_capacity,hi_beds,hi_deposit,hi_min_days,hi_max_days) " \
            ...
            house_id = self.db.execute(sql, user_id=user_id,
                                       title=title,
                                       price=price,
                                       area_id=area_id,
                                       address=address,
            ...
            logging.error(e)
            return self.write(dict(errcode=RET.DBERR, errmsg="保存數據失敗"))

        try:
            sql = "insert into ih_house_facility(hf_house_id,hf_facility_id) values"
            sql_val = []  # 用來保存條目的(%s,%s)部分,最終形式["(%s,%s)","(%s,%s)"]
            vals = []  # 用來保存的具體的綁定變量值
    ...
    def get(self):
        """獲取房屋信息"""
        sess = Session(self)
        user_id = sess.data.get("user_id", "-1")
        house_id = self.get_argument("house_id")
        # 校验参数
        if not house_id:
        ...
        # 先从redis缓存中获取信息
        try:
            ret = self.redis.get("house_inof_%s" % house_id)
        except Exception as e:
            logging.error(e)
            ret = None
        if ret:
            # 此时从redis中获取到的是缓存的json格式数据
            return self.write(dict(errcode=RET.OK, errmsg="OK", data=ret, user_id=user_id))

        # 查询房屋基本信息
        sql = "select hi_title,hi_price,hi_address,hi_room_count,hi_acreage,hi_house_uint,hi_capacity,hi_beds," \
        ...
        try:
            ...
            logging.error(e)
        return self.write(dict(errcode=RET.OK, errmsg="OK", data=data, user_id=user_id))
...
class IndexHandler(BaseHandler):
    ...
    def get(self):
        try:
            ...
            ret = None
        if ret:
            json_houses = json.loads(ret)
        else:
            try:
                # 查询数据库，返回房屋订单数目最多的5条数据（房屋订单通过hi_order_count来表示）
                house_ret = self.db.query("select hi_house_id,hi_title,hi_order_count,hi_index_image_url "
                                          "from  ih_house_info order by hi_order_count desc limit %s;" \
                                          % constants.HOME_PAGE_MAX_HOUSES)
            except Exception as e:
                logging.error(e)
                return self.write(dict(errcode=RET.DBERR, errmsg="获取数据失败"))
            if not house_ret:
                return self.write(dict(errcode=RET.DBERR, errmsg="没有数据"))
            houses = []
            for l in house_ret:
...
class HouseListHandler(BaseHandler):
    """房源列表页面"""
    """get方式获取数据库数据，本身不会对数据库产生影响
    参数说明：
    start_date 用户查询其实时间 sd  非必传 ""
    end_data 用户查询其实时间 ed  非必传 ""
    area_id 用户查询其实时间 aid  非必传 ""
    sort_key 用户查询其实时间 sk  非必传 ""
    page 用户查询其实时间 p  非必传 ""
    """

    def get(self):
        start_date = self.get_argument("sd", "")
        end_data = self.get_argument("ed", "")
        area_id = self.get_argument("aid", "")
        sort_key = self.get_argument("sk", "ner")
        ...
        # 数据查询
        # 涉及到表：
        # ih_house_info 房屋基本信息
        # hi_user_profile 房东的用户信息
        # ih_order_info 房屋订单数据
        ...
        sql_total_count = "select count(distinct hi_house_id) from ih_house_info " \
                          "inner join ih_user_profile on hi_user_id=up_user_id " \
                          "left join ih_order_info on hi_house_id=oi_house_id"
...
class HouseListRedisHandler(BaseHandler):
    ...
    def get(self):
        ...
        # 先从redis获取数据
        try:
            redis_key = "houses_%s_%s_%s_%s" % (start_date, end_data, area_id, sort_key)
        ...
        house_date = {}
        house_date[page] = json.dumps(dict(errcode=RET.OK, errmsg="OK", data=current_page_data, total_page=total_page))
        # 将多取出来的数据分页
        i = 0
        while 1:
            page_date = data[i * constants.HOUSE_LIST_PAGE_CAPACITY:(i + 1) * constants.HOUSE_LIST_PAGE_CAPACITY]
            if not page_date:
                break
            house_date[page + i] = json.dumps(dict(errcode=RET.OK, errmsg="OK", data=page_date, total_page=total_page))
            i += 1
        try:
            redis_key = "house_%s_%s_%s_%s" % (start_date, end_data, area_id, sort_key)
            self.redis.hmset(redis_key, house_date)
            self.redis.expire(redis_key, constants.REDIS_HOUSE_LIST_EXPIRES_SECONDS)
        except Exception as e:
            logging.error(e)
        self.write(house_date[page])
