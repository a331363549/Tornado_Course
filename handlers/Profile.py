...
class AvatarHandler(BaseHandler):
    ...
    def post(self):
        ...
        if not files:
            return self.write(dict(erroce=RET.PARAMERR, errmsg="未上传图片"))
        avatar = files[0]["body"]
        ...
        self.write(dict(erroce=RET.OK, errmsg="保存成功", data="%s%s" % (constants.QINIU_URL_PREFIX, file_name)))
...
class ProfileHandler(BaseHandler):
    ...
    def get(self):
        ...
        try:
            ...
            logging.error(e)
            return self.write(dict(errcode=RET.DBERR, errmsg="获取个人信息失败"))
        if ret["up_avatar"]:
            img_url = constants.QINIU_URL_PREFIX + ret["up_avatar"]
        else:
            img_url = None
        self.write(dict(errcode=RET.OK, errmsg="OK", data=dict(user_id=user_id,
                                                               name=ret["up_name"],
...
class AuthHandler(BaseHandler):
    ...
    def get(self):
        ...
        try:
            ...
            # 数据库查询出错
            logging.error(e)
            return self.write(dict(errcode=RET.DBERR, errmsg="数据获取失败"))
        logging.debug(ret)
        if not ret:
            return self.write(dict(errcode=RET.NODATA, errmsg="用户不存在"))
        self.write(dict(errcode=RET.OK, errmsg="OK", data=dict(
    ...
    def post(self):
        ...
        try:
            ...
            logging.error(e)
            return self.write(dict(errcode=RET.DBERR, errmsg="更新失败"))
        self.write(dict(errcode=RET.OK, errmsg="OK"))
